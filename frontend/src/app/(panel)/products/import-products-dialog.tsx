...
import { productKeys } from "@/lib/api/queries/products";
...
function downloadTemplate() {
  ...
  a.href = "/api/proxy/imports/templates/products";
  a.download = "plantilla_products.xlsx";
  a.target = "_blank";
...
export function ImportProductsDialog() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await fetch("/api/proxy/imports/products", {
        method: "POST",
        body: fd,
      ...
      const payload = (await res.json().catch(() => null)) as
        ...
        | null;
      if (!res.ok) {
        throw new Error(payload?.message ?? "No se pudo importar el archivo.");
      }
      ...
      if (failed > 0) {
        ...
        toast.success(result.message ?? "Productos importados correctamente");
        setOpen(false);
        reset();
      }
    } catch (err) {
    ...
    >
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Sube un archivo CSV o Excel con tus productos. Usa la plantilla para
          ...
          <Field label="Archivo (.csv / .xlsx)">
            <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-dashed border-input bg-card px-4 py-4 text-sm transition hover:border-primary/40">
              ...
              <span className={file ? "font-medium" : "text-muted-foreground"}>
                {file ? file.name : "Selecciona tu archivo de productos"}
              </span>
              <input
                type="file"
                accept=".csv,.xlsx"
