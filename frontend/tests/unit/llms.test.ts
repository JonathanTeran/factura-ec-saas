...
describe("llms.txt", () => {
  const text = buildLlmsTxt({ appUrl: "https://facturon.ec", plans: LANDING_FIXTURE.plans, contactEmail: "info@facturon.ec", whatsapp: "+1 334 732 4056" });
  ...
  it("sigue el formato llmstxt.org y solo contiene hechos", () => {
    ...
    expect(text).toContain("info@facturon.ec");
  ...
  it("sin planes omite la sección de precios", () => {
    const t = buildLlmsTxt({ appUrl: "https://facturon.ec", plans: [], contactEmail: "info@facturon.ec", whatsapp: "+1 334 732 4056" });
