...
$icons = [
    ...
    'receipt-refund' => '<path stroke-linecap="round" stroke-linejoin="round" d="M8.25 9.75h4.875a2.625 2.625 0 010 5.25H12M8.25 9.75L10.5 7.5M8.25 9.75L10.5 12m9-7.243V21.75l-3.75-1.5-3.75 1.5-3.75-1.5-3.75 1.5V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0c1.1.128 1.907 1.077 1.907 2.185z" />',
    ...
    'document-check' => '<path stroke-linecap="round" stroke-linejoin="round" d="M10.125 2.25h-4.5c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125v-12.75M10.125 2.25h.375a9 9 0 019 9v.375M10.125 2.25A3.375 3.375 0 0113.5 5.625v1.5c0 .621.504 1.125 1.125 1.125h1.5a3.375 3.375 0 013.375 3.375M9 15l2.25 2.25L15 12" />',
...
$baseClasses = 'group relative flex items-center gap-x-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200';

if ($variant === 'highlight') {
    $activeClasses = 'bg-primary-600 text-white shadow-lg shadow-primary-600/25';
    $inactiveClasses = 'bg-primary-500/10 text-primary-400 hover:bg-primary-600 hover:text-white hover:shadow-lg hover:shadow-primary-600/25';
} else {
    $activeClasses = 'bg-white/[0.1] text-white';
    $inactiveClasses = 'text-slate-400 hover:bg-white/[0.06] hover:text-slate-200';
}
...
<a href="{{ $href }}"
   class="{{ $classes }}"
   @if($collapsed) x-tooltip.right="'{{ $slot }}'" @endif>
    {{-- Active indicator --}}
    @if($active && $variant !== 'highlight')
    <span class="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-r-full bg-primary-400"></span>
    @endif

    <svg class="h-5 w-5 shrink-0 {{ $active ? '' : 'text-slate-500 group-hover:text-slate-300' }} transition-colors duration-200"
         fill="none"
         viewBox="0 0 24 24"
         stroke-width="1.5"
         stroke="currentColor">
    ...
    <span x-show="!sidebarCollapsed || {{ $collapsed ? 'false' : 'true' }}"
          x-transition:enter="transition ease-out duration-200"
          x-transition:enter-start="opacity-0"
          x-transition:enter-end="opacity-100"
          class="truncate">{{ $slot }}</span>
    @if($badge !== null)
    <span x-show="!sidebarCollapsed || {{ $collapsed ? 'false' : 'true' }}"
          class="ml-auto inline-flex items-center rounded-md bg-white/[0.08] px-1.5 py-0.5 text-[11px] font-semibold text-slate-300 tabular-nums">
        {{ $badge }}
    </span>
    @endif
</a>
